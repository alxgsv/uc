class UploadcareWebhooksService
  ...
  def setup_uploadcare_webhooks
    ...
    uploadcare_webhook_params.each do |params|
      if existing_webhooks.find { |webhook| webhook.target_url == params[:target_url] }.blank?
        Uploadcare::Webhook.create(params)
      end
    end
  end

  def uploadcare_webhook_params
    EVENTS.map do |event|
      {
